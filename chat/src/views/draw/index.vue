...
import {
  fetchChatDraw,
  fetchGetAllChatLogDraw,
  fetchGetChatLogDraw,
} from '@/api';
import { useBasicLayout } from '@/hooks/useBasicLayout';
import { TitleBar } from '@/components/base';
import { useAppStore, useAuthStore } from '@/store';
import GridManager from '@/components/common/DalleGridManager/index.vue';
...
async function queryAllDrawList() {
  const res: ResData = await fetchGetAllChatLogDraw({
    size: 999,
    rec: 1,
    model: 'dall-e-3',
  });
...
<template>
  ...
  >
    ...
    <div :class="isMobile ? ['px-2'] : ['px-20']">
      ...
      <div class="flex my-5">
        <b
          class="text-primary cursor-pointer select-none flex-shrink-0"
          @click="updateEx"
          >换示例</b
        >
      ...
      <div v-if="loading" class="mt-8 pb-10">
        ...
        <div class="flex flex-wrap mt-8">
          <div
            class="w-44 h-44 border rounded-md relative ml-4 mt-4"
            v-for="i in form.n"
            :key="i"
          >
            <Loading
