...
import { EllipsisHorizontalIcon, CheckIcon } from '@heroicons/vue/24/outline';
...
async function handleEnter(params: Chat.History, event: KeyboardEvent) {
  event?.stopPropagation();
  if (event.key === 'Enter') updateGroupInfo(params);
}
...
<template>
  ...
  <div v-for="item of dataSources" :key="`${item.uuid}`">
    <div
      class="relative flex items-center gap-3 px-3 py-2 break-all rounded-md cursor-pointer hover:bg-gray-50 group dark:hover:bg-gray-800 font-medium text-sm"
      :class="
        isActive(item.uuid)
          ? [
              'bg-gray-50',
