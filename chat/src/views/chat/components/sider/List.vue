...
const appList = computed(() =>
  dataSources.value.filter((item) =>
    groupKeyWord.value
      ? item.title.includes(groupKeyWord.value) && !item.isSticky && item.appId
      : !item.isSticky && item.appId
  )
);
  ...
  dataSources.value.filter((item: any) => {
    if (groupKeyWord.value)
      return (
        ...
        !item.isSticky &&
        !item.appId &&
        formatUtcTime(item.createdAt) >= today
      );
    else
      return (
        !item.isSticky && !item.appId && formatUtcTime(item.createdAt) >= today
      );
  })
);
