...
function handleToPage(event: MouseEvent, menu: MenuItem) {
...
function isActive(item: MenuItem) {
  ...
  if (menuIframeUrl) return menuIframeUrl === iframeSrc.value;

  if (menuPath) return menuPath === activeRoutePath.value;
}
  ...
  <footer class="bg-white dark:bg-[#25272c]">
    ...
    >
      <a
        ...
        @click="handleToPage($event, item)"
      >
        <span class="inline-block text-xl">
          <SvgIcon :icon="item.menuIcon" class="mb-1 inline-block text-lg"
        /></span>
