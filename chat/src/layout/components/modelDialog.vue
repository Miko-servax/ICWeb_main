...
const disabled = computed(() => {
  return (
    Number(activeConfig.value?.modelTypeInfo?.val) !== 1 ||
    Number(activeGroupAppId.value) > 0
  );
});
...
function compilerConfig(val: any) {
  ...
  rounds.value =
    modelInfo.rounds > modelInfo.maxRounds
      ? modelInfo.maxRounds
      : modelInfo.rounds;
}
...
const options = computed(() => {
  const data = !activeGroupAppId.value
    ? modelTypeListCache
    : modelTypeListCache.value.filter((item: any) => Number(item.val) === 1);
...
function getModelTypeInfo(type: any) {
  return modelTypeListCache.value.find((item: any) => item.val === type);
}
    ...
    <div class="py-3 px-5 bg-white rounded dark:bg-slate-800">
      ...
      <div class="mt-5 bg-[#fafbfc] px-2 py-2 dark:bg-[#243147]">
        <n-collapse
          ...
          accordion
          :on-update:expanded-names="handleUpdate"
        >
          <n-collapse-item name="1">
            ...
            <template #header-extra>
              <div @click.stop="handleReset">
                <NButton v-if="showResetBtn" text type="error"> 重置 </NButton>
              </div>
            </template>
            <div class="mt-2">
              <div>
              ...
              <div class="mt-4">
                <div class="w-full flex justify-between">
                  ...
                  <div class="flex w-[200px] items-center">
                    <n-slider
                      v-model:value="maxResponseTokens"
                      :step="100"
                      :max="maxModelTokens"
                    />
                    <span class="w-[55px] text-right">
